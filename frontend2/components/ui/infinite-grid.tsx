...
export const InfiniteGrid = ({ className }: { className?: string }) => {
  ...
  return (
    ...
    >
      <div className="absolute inset-0 opacity-[0.08] text-[#003566]">
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} />
      </div>
      <motion.div
        className="absolute inset-0 opacity-35 text-[#003566]"
        style={{ maskImage, WebkitMaskImage: maskImage }}
      >
        <GridPattern offsetX={gridOffsetX} offsetY={gridOffsetY} />
      </motion.div>

      {/* ambient glow — subtle blue for light mode */}
      <div
        className="absolute inset-0"
        style={{
          background:
            "radial-gradient(ellipse 60% 50% at 50% -5%, rgba(0,53,102,0.4) 0%, transparent 70%)",
        }}
      />
    </div>
  );
